...
function getRoundPhase(round: RoundRow | null) {
  if (!round) return 'none' as const
  if (round.status === 'ended') return 'ended' as const
  const now = Date.now()
  const start = new Date(round.start_at).getTime()
  const end = new Date(round.end_at).getTime()
  if (Number.isFinite(start) && now < start) return 'planned' as const
...
export default async function ReleaseVotingPage({
  ...
  async function submitVoteAction(formData: FormData) {
    ...
    if (error) redirect(`/release-voting?error=${encodeURIComponent(error.message)}`)
  ...
  const phase = getRoundPhase(currentRound)
    ...
    <main className="min-h-screen bg-zinc-50 p-6 md:p-10">
      <div className="mx-auto max-w-5xl space-y-6">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-bold tracking-tight text-zinc-900">{currentRound?.title ?? 'Neue Songs der Woche'}</h1>
          <p className="mt-3 text-zinc-600">{currentRound?.description ?? 'Vergib 12 bis 1 Punkte. Jeder Song darf nur einmal vorkommen.'}</p>
        </div>

        {ok === 'saved' && (
          <div className="rounded-2xl border border-green-200 bg-green-50 px-4 py-3 text-green-800">
            Deine Wertung wurde gespeichert.
          </div>
        )}
          ...
          <>
            <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
              <div className="flex flex-wrap gap-3 text-sm font-medium">
                <span className={`rounded-full px-4 py-1 ${phase === 'live' ? 'bg-green-100 text-green-800' : phase === 'planned' ? 'bg-amber-100 text-amber-800' : 'bg-zinc-100 text-zinc-700'}`}>
                  Status: {phase === 'live' ? 'Live' : phase === 'planned' ? 'Geplant' : 'Beendet'}
                </span>
                <span className="rounded-full bg-zinc-100 px-4 py-1 text-zinc-700">Start: {formatDateTime(currentRound.start_at)}</span>
                <span className="rounded-full bg-zinc-100 px-4 py-1 text-zinc-700">Ende: {formatDateTime(currentRound.end_at)}</span>
                <span className="rounded-full bg-zinc-100 px-4 py-1 text-zinc-700">Stimmen: {votes.length}</span>
              </div>
            </section>

            {phase === 'planned' && (
              <section className="rounded-3xl border border-zinc-200 bg-white p-10 text-center shadow-sm">
                <h2 className="text-3xl font-semibold text-zinc-900">Die Abstimmung startet bald.</h2>
                <p className="mt-4 text-lg text-zinc-600">Das Voting beginnt am {formatDateTime(currentRound.start_at)}.</p>
              </section>
            )}

            {phase === 'ended' && (
              <section className="rounded-3xl border border-zinc-200 bg-white p-10 text-center shadow-sm">
                <h2 className="text-3xl font-semibold text-zinc-900">Die Abstimmung ist beendet.</h2>
                <p className="mt-4 text-lg text-zinc-600">Du kannst unten das Endergebnis sehen.</p>
              </section>
            )}
                ...
                <form action={submitVoteAction} className="mt-6 space-y-5">
                  ...
                  <div className="grid gap-5 md:grid-cols-3">
                    <div>
                      ...
                      <input name="juror_name" required className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-medium text-zinc-700">E-Mail</label>
                      <input name="juror_email" type="email" className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-medium text-zinc-700">Instagram</label>
                      <input name="juror_instagram" className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    {POINTS.map((points) => (
                      <div key={points} className="grid gap-3 md:grid-cols-[110px,1fr] md:items-center">
                        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-center font-semibold text-zinc-900">{points} Punkte</div>
                        <select name={`song_${points}`} required className="w-full rounded-2xl border border-zinc-300 px-4 py-3 outline-none focus:border-zinc-900">
              ...
              <div className="mt-6 overflow-x-auto">
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {results.length === 0 && (
                      <tr>
                        <td colSpan={5} className="py-4 text-zinc-500">Noch keine Ergebnisse vorhanden.</td>
                      </tr>
                    )}
                    {results.map((row) => (
